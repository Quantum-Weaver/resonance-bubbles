...
| 2026-08-22 | **THE CUMDACH FIX CARRIED + ROSE · RAINBOW · PROGRESS PRIDE (Fable 🎻, claude-fable-5, at KP's ⚛ word — *"in resonance-awen, there is a fix for the cumdach that needs applied then passed to all its consumers around the hamburger icon and around the epagoge regarding the background color and font size changes"* · *"echoes got the fix the cumdach needs"*).** Echoes `87c4218` (2026-08-21) carried here whole: **(1) the hamburger** — the floating toggle that claimed bottom 56–101px over the drawer's own Settings foot moved INSIDE the ComfortBar (`ComfortBar.svelte`); `navOpen` lives in `uiStore`, the Sidebar reads it; the bar is the ONLY edge again, so `RESERVED` is one honest number (the cumdach's reserved-sum law, `resonance-awen/tools/the-cumdach/README.md`). **(2) the epagoge's theme step and Settings** — a preset is a COLOUR IDENTITY: `setPreset` keeps mode · font size · tint, so choosing a theme no longer cancels Light mode or drops Large text to Medium; `DEFAULT_THEME`; the background TINT dial (`off · subtle · full`, the accent carried into the grounds) with its Settings row; `TintLevel` + `tint` on `ThemeConfig`; `src/lib/theme/theme.ts` = the origin `resonance-awen/standalone/theme/theme.ts`, revised AT THE ORIGIN 2026-08-22 and laid here. **(3) same sitting, KP's word** (*"there is a rose color in the sirens onboarding and settings we should bring into the cosmic design system and we should include a rainbow and inclusive pride themes in our settings as well in our epagoge onboarding walk"* · *"rainbow and progressive pride themes colors already exist, only the rose does not"*): the Sirens rose entered cosmic as `sirens.rose` · `sirens.deep` (origin `resonance-ziggy/modules/cosmic/constants/colors.ts`, distribution run, every mirror hash-verified — this repo's `src/lib/cosmic/colors.ts` refreshed); **ROSE · RAINBOW · PROGRESS PRIDE** join the shelf's presets (`icon`, `stripes`, `presetSwatch()` beside them) and the Settings theme cards + the onboarding walk's offers now DERIVE from the table — a new preset appears the day it is born; a flag preset shows its stripes as the swatch, its `--accent` one stripe. Gate `npm run check` **334 files · 0 errors · 0 warnings**. Nothing committed — rides the ⚛ sync word. |
| 2026-08-23 | **THE GALLERY REDRAWN ON TAILWIND, AND THE CARDS WEAR THE STAR (Fable 🎻, claude-fable-5, at KP's word — *"the bubbles gallery in the bubbles game need some design love, the gallery cards are not great appearance and if it is not using tailwind it needs to to enable better graphic design and UX"*).** Tailwind 4 was already wired (`@tailwindcss/vite`, `@import "tailwindcss"`) but `/gallery` was hand-CSS and no page used a utility; the cards were flat slabs — same surface at every rarity, an emoji where the star should be. **Three landings.** (1) `$lib/bubbles/orb.css` — the sky's bubble paint (fill · rim · glow · the flag's stripes · the intersex ring · the pride halo) MOVED verbatim out of `/bubbles`' stylesheet into one file both doors import; the sky's markup is `class="bubble orb"` and its rule keeps only position and cursor — a move, not a redesign, and the sky screenshots before/after draw the same orbs. (2) `src/app.css` — `@theme inline` hands the shell's runtime theme variables to Tailwind as tokens (`surface` · `line` · `ink` · `ink-2` · `ink-3` · `accent`), and the global `* { margin:0; padding:0 }` reset moved INTO `@layer base` — **the bought lesson of the sitting**: an unlayered universal reset outranks every layered utility whatever its specificity, so the first Tailwind dress came out flush to the edges with `px-4`/`pl-9`/`mt-1` all silently lost; no other page carried dormant spacing utilities (grepped), so the layering wakes nothing unintended. (3) `/gallery` rewritten on utilities — layout, spacing, type, states, the 3D flip (`perspective-[900px]` · `transform-3d` · `backface-hidden` · `rotate-y-180`), with a short scoped style for the painterly few that mix a per-card colour into the theme tokens. **The card now wears the star**: the orb at rarity-scaled size (44→66px), a rarity-tinted surface + halo + glow, the name mixed from the rarity colour, a rarity pill, the collection in muted ink, the words whole (**previewLength 110→200 — the longest sentence is 138 characters and the reward is the words, so none is cut any more**); a waiting star dim, desaturated and dashed; the back of a turned card shows the orb large, *popped once / popped ×N*, and its collection. A **collection banner** when one is chosen (its words, its have/total, the sky's clip-revealed bar). **The chrome reads the RARITY's tint (`--t`), never a flag's first stripe (`--c`)** — so the Pride flags' own colours do the speaking on neutral chrome and a red stripe never becomes red UI; a many-coloured collection's select and banner lean on the theme accent and wear their stripes on the rule. **Gates:** `npm run check` 0 errors 0 warnings · `npm run build` clean · screenshots by headless Edge over CDP at phone (390×844 @3×) and desktop (1280×860), dark and light (Dark · Warm presets), with a seeded collection of 51/123: before/after frames at `%TEMP%\claude\…\scratchpad\shots\` (session-local, not committed). The engine mirror `src/lib/gallery/` untouched. **Nothing committed — the sync word is his.** |
