<script lang="ts">
	...
	// The wordmark law, applied at this door too: the title reads the app's
	// OWN name (productName in tauri.conf.json is the single truth), so a
	// rename never gets chased into the chrome. Everything beneath the name
	// is echoes-within-bubbles by design — KP's word: "it is literally
	// intended to be echoes, with bubbles."
	...
	// Compass its first consumer). The app brings the particulars into the
	// walk's slots; the walk owns the flow, the dots, the honest record.
	...
	// (KP's word: "it adds the other available styles choices").
	...
	// changeable in Settings. The vessel-name key is echoes' own — kept
	// whole by KP's word.
	function finish() {
