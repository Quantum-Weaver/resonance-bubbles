<script lang="ts">
	...
	const PRIVACY_URL = 'https://github.com/Quantum-Weaver/resonance-bubbles/blob/main/PRIVACY.md';
	...
	async function exportData() {
		...
		// The GAME's ground, carried on the same envelope. The bubble
		// collection, the daily boundary and the sound preference all live
		// under one localStorage key ('the-bubbles'), and executePurge calls
		// localStorage.clear() — so without this line the purge destroys
		// something the export never carried, and "purge and export cover
		// exactly the same ground" would quietly stop being true.
		let collections: unknown = null;
		try {
			collections = JSON.parse(localStorage.getItem('the-bubbles') || 'null');
		} catch {
			collections = null;
		}
		const collectedRecord = (collections as { collected?: Record<string, number> } | null)?.collected;
		const collectedCount = collectedRecord ? Object.keys(collectedRecord).length : 0;
		const payload = {
			envelope: 'resonance-export',
			envelopeVersion: 1,
			app: 'resonance-bubbles',
			appVersion: appVersion || 'unknown',
			exportedAt: new Date().toISOString(),
			counts: {
				echoes: allEchoes.length,
				folksonomy: Object.keys(folksonomy).length,
				bubbles: collectedCount
			},
			data: { echoes: allEchoes, folksonomy, collections }
		};
		...
		a.href = url;
		a.download = `resonance-bubbles-export-${date}.json`;
	...
	async function handleImportFile(ev: Event) {
		...
		try {
			...
			if (Array.isArray(parsed)) {
				...
				if (parsed.app !== 'resonance-bubbles') {
					throw new Error(
						`This file belongs to ${parsed.app ?? 'another app'} — Bubbles imports only its own envelopes.`
					);
				}
				...
				throw new Error('Not a Resonance Bubbles export file.');
			}
...
<div class="settings" style="padding-top: env(safe-area-inset-top, 0px);">
	...
	<section class="section">
		...
		<div class="about-card">
			<div class="about-app">
				<span class="about-name">Resonance Bubbles</span>
